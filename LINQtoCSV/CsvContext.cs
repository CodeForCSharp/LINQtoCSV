...
namespace LINQtoCSV
{
    ...
    public class CsvContext
    {
        ...
        ///     Returns the data into an IEnumerable
        ///     <T>
            ...
            CsvFileDescription fileDescription) where T : class, new()
        {
            ...
            var readingRawDataRows = typeof(IDataRow).IsAssignableFrom(typeof(T));
            ...
            var cs = new CsvStream(stream, null, fileDescription.SeparatorChar,
            ...
            try
            {
                ...
                while (cs.ReadRow(row, charLengths))
                {
                    ...
                    if (row != null && row.Count == 1 && (row[0].Value == null || string.IsNullOrEmpty(row[0].Value.Trim())))
                        continue;
